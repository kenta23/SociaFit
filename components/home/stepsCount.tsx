...
export default function StepsCount() {
	...
	const calculateSteps = async (
		...
		type: "week" | "month" | "today",
	): Promise<number | undefined> => {
		const pastStepCountResult = dateRanges.week
	...
	const subscribe = async (): Promise<Subscription | undefined> => {
		...
		if (isAvailable) {
			const getResultsData = await Promise.all([
				calculateSteps(dateRanges, "today"),
				calculateSteps(dateRanges, "week"),
				calculateSteps(dateRanges, "month"),
			]);
			//store to the database
			if (getResultsData[0] !== undefined) {
				const { data: monthlyStepsNewData } = await supabase
					.from("userdata")
					.update({ monthly_steps: getResultsData[0] })
					.eq("user_id", user.data.user?.id as string)
					.select()
					.single();
				if (monthlyStepsNewData) {
					storeData(
						monthlyStepsNewData as Database["public"]["Tables"]["userdata"]["Row"],
					);
				}
			} else if (getResultsData[1] !== undefined) {
				const { data: weeklyStepsNewData } = await supabase
					.from("userdata")
					.update({ weekly_steps: getResultsData[1] })
					.eq("user_id", user.data.user?.id as string)
					.select()
					.single();
				if (weeklyStepsNewData) {
					storeData(
						weeklyStepsNewData as Database["public"]["Tables"]["userdata"]["Row"],
					);
				}
			} else if (getResultsData[2] !== undefined) {
				const { data: todayStepsNewData } = await supabase
					.from("userdata")
					.update({ today_steps: getResultsData[2] })
					.eq("user_id", user.data.user?.id as string)
					.select()
					.single();
				if (todayStepsNewData) {
					storeData(
						todayStepsNewData as Database["public"]["Tables"]["userdata"]["Row"],
					);
				}
			} else {
	...
	const stepsCountData = useMemo<StepData[]>(() => {
		const rawPercent =
			targetSteps === 0 ? 0 : (currentSteps / targetSteps) * 100;
	...
	const footstepsIcon = useMemo(() => {
		return stepsCountData[0].value > 0
			? require("@/assets/images/footsteps.svg")
			: require("@/assets/images/footsteps-dark.svg");
	}, [stepsCountData]);
