...
100 kg → ~0.06 kcal/step*/
	...
	const caloriesPerStep: number =
		weight === 60 ? 0.04 : weight === 80 ? 0.05 : weight === 100 ? 0.06 : 0;
	const caloriesBurnt: number = useMemo(
		() =>
			data?.today_steps
				? data.today_steps * (data?.ds_travelled ?? 0) * caloriesPerStep
				: 0,
		[data?.today_steps, caloriesPerStep, data?.ds_travelled],
	);

	useEffect(() => {
		async function getData() {
			const user = await supabase.auth.getUser();
			const { data: healthDetailsData, error: healthDetailsError } =
				await supabase
					.from("health_details")
					.select("*")
					...
					<View style={styles.stepsCountParent}>
						...
						<Text
							style={[
								typography.heading,
								styles.text,
								{ color: Colors[colorScheme].yellow["600"] },
							]}
						>
							{data?.today_steps ?? 0}
						</Text>
			...
			<View style={styles.frameParent}>
				<View style={styles.frameGroup}>
					<View style={styles.stepsCountParent}>
						...
						>
							{convertCaloriesToEnergyUnit(caloriesBurnt, units.energyUnits)}
						</Text>
					</View>
					<Text
						style={[
					...
					>
						{formatEnergyValue(caloriesBurnt, units.energyUnits)}
					</Text>
				</View>
			</View>
		</View>
