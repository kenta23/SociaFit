...
interface UserDataProps { 
    authSession: AuthResponse['data']['user'] | null;
    following: Database['public']['Tables']['following']['Row'][] | null;
    totalLikes: number;
    activities: Database['public']['Tables']['activities']['Row'][] | null;
...
export default function ProfileVisit() {
    ...
    useEffect(() => { 
         const getUser = async () => { 
              ...
              const { userData, following, likes, activities } = await getUserData(userid as string);
              ...
              <View style={styles.view}>
                  ...
                  <View style={[styles.frameParent, styles.frameFlexBox]}>
                      ...
                      <View style={styles.nameContainer}>
                          <Text style={[typography.heading]}>Unknown User</Text>
                          <Text style={typography.medium}>{userdata[0].authSession?.email}</Text>
                      </View>
                  </View>
                  ...
                  <View style={[styles.frameGroup, styles.frameFlexBox]}>
                      <View style={[styles.groupFlexBox]}>
                          <Text style={[typography.medium]}>{userdata[0].totalLikes}</Text>
                          <Text style={[typography.description]}>Total likes</Text>
                      </View>

                      <View style={[styles.groupFlexBox]}>
                          <Text style={[typography.medium]}>{userdata[0].totalActivities}</Text>
                          <Text style={[typography.description]}>Activities</Text>
                      </View>

                      <View style={styles.groupFlexBox}>
                          <Text style={[typography.medium]}>{userdata[0].following?.length || 0}</Text>
