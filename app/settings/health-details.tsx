...
import {
  convertHeightToHeightUnit,
  formatHeightValue,
} from "@/utils/unitsconversion";
import BottomSheet, {
  BottomSheetBackdrop,
...
const WEIGHT_VALUES: number[] = Array.from(
	{ length: 454 },
	(_, index) => index + 1,
);
const HEIGHT_VALUES: number[] = Array.from(
	{ length: 250 },
	(_, index) => index + 30,
);
const GENDER_VALUES: string[] = ["Male", "Female", "Not Set", "Other"];
const AGE_VALUES: number[] = Array.from(
	{ length: 100 },
	(_, index) => index + 1,
);
...
export default function HealthDetails() {
	...
	const [storeBottomSheetContent, setStoreBottomSheetContent] = useState<
	...
	async function handleUpdateHealthDetails(item: string, index: number) {
		try {
			...
			const dataToUpdate: Database["public"]["Tables"]["health_details"]["Insert"] =
				{
			...
			//set BMI
			const weight = dataToUpdate.weight
				? dataToUpdate.weight
				: healthDetails?.weight;
			const height = dataToUpdate.height
				? dataToUpdate.height
			...
			if (data) {
				...
				setHealthDetails(
					data as Database["public"]["Tables"]["health_details"]["Row"],
				);
			}
	...
	const renderItem = useCallback(
		(item: { id: number; value: number[] | string[] }) => (
			<View key={item.id}>
				{item.value.map((i, idx) => {
					...
					if (item.id === 0) text += " kg";
						...
						text =
							convertHeightToHeightUnit(i as number, units.heightUnits) +
							" " +
							(units.heightUnits === "Centimeters" ? "cm" : "m");
					} else text;
					const lowOpacity =
						item.id === 0
							? healthDetails?.weight === i
								? 0.5
								: 1
							: item.id === 1
								? healthDetails?.height === i
									? 0.5
									: 1
								: item.id === 2
									? healthDetails?.age === i
					...
					return (
						...
						>
							<Text
								...
								style={[
									typography.heading,
									{ color: Colors[colorScheme].text["600"] },
								]}
							>
								{text}
							</Text>
		...
		>
			<GestureHandlerRootView>
				<View style={containerStyles.container}>
					<View style={styles.contentContainer}>
						...
						<View style={{ flexDirection: "column", gap: 4 }}>
							...
							>
								Changing your information must be detailed as possible{" "}
							</Text>
						</View>
								...
								return (
									...
									>
										<Pressable
											...
											onPress={() => handleBottomSHeetContent(index)}
										>
											<View>
												<View
													style={{
												...
												>
													...
													>
														{index === 0
															? `${healthDetails?.weight || 0} kg`
															: index === 1
																? formatHeight()
																: index === 2
																	? `${healthDetails?.age || 0} years`
																	: healthDetails?.gender || "N/A"}
													</Text>
												</View>
						...
						>
							...
							>
								...
								>
									{`${healthDetails?.BMI || 0} - ${
										healthDetails?.BMI
											? healthDetails?.BMI < 18.5
												? "Underweight"
												: healthDetails?.BMI < 24.9
													? "Normal"
													: healthDetails?.BMI < 29.9
														? "Overweight"
														: "Obesity"
											: "N/A"
									}`}
				...
				>
					<View>
						<BottomSheetScrollView
							style={{ backgroundColor: Colors[colorScheme].frameBackground }}
						>
